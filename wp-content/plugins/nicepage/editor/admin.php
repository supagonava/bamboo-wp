...
class NpAdminActions {
     ...
     * @return array $result
     */
    public static function getPostsByCategories($type) {
        $sources = array();
        $productSources = array();
        if ($type === 'blog') {
            $sources = self::getPostsCategories('blog');
        } else if ($type === 'product') {
            $sources = self::getPostsCategories('products');
            $productSources = self::getPostsCategories('product');
            if (function_exists('wc_get_product') && !wc_get_product($productSources[0])) {
                $productSources = array();
            }
        }
     ...
     * @return array
     */
    public static function getPostsCategories($type = 'blog') {
        global $post;
        $data_provider = np_data_provider($post->ID);
    ...
    public static function getPostsByCategory($category, $type = 'blog') {
        ...
        $items = self::getPosts($category, 20, $postType);
        foreach ($items as $key => $item) {
            if ($type == 'blog') {
                $post = self::getBlogPost($item);
            } else {
     ...
     * @return array $data
     */
    public static function getBlogPost($item) {
        // post image
        ...
        $data = array(
            ...
            'post-content' => plugin_trim_long_str(self::getTheExcerpt($item->ID), 150),
    ...
    public static function printBridgeAction() {
        ...
        $data['info'] = array(
            ...
            'blogInfo' => self::getPostsByCategories('blog'),
            'videoFiles' => self::getVideoFiles(),
            'productsInfo' => self::getPostsByCategories('product'),
        );
