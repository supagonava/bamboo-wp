...
class NpContentImporter {
    ...
    private function _updatePostType($post_type, &$data) {
        ...
        foreach ($data as $id => $post_data) {
            ...
            $update_data = array(
                'ID' => $post_id,
                'post_content' => str_replace('<!--CUT-->', '<!--more-->', $this->_processContent(_arr($post_data, 'content', ''))),
    ...
    private function _importParameters($onlyHeaderFooter = false) {
        ...
        if (isset($data['cookiesConsent'])) {
            $data['cookiesConsent']['publishCookiesSection'] = np_data_provider()->replaceImagePaths($data['cookiesConsent']['publishCookiesSection']);
            NpMeta::update('cookiesConsent', $this->_processContent($data['cookiesConsent']));
        }
    }
